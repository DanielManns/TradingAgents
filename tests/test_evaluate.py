...
from tradingagents.portfolio.persistence import archive_rebalance, load_portfolio, save_portfolio
...
def test_portfolio_spy_none_when_unavailable():
    _, spy_return = evaluate_portfolio(
        [10.0], date="2024-01-01", price_fetcher=lambda t, d: None if t == "SPY" else 110.0
    )
...
def test_history_twr_calculation(tmp_path, returns, expected_twr):
    ...
    for date, r in zip(dates, returns, strict=False):
